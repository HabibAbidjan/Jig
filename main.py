...
# === /addbal ID SUMMA ===
addbal_state = {}  # Adminning holatini saqlaydi
...
def handle_callback(call):
    ...
    if call.data == "cashout":
        ...
        user_balances[user_id] += win
        del user_games[user_id]
        bot.edit_message_text(f"✅ {win} so‘m yutdingiz!", call.message.chat.id, call.message.message_id)
    elif call.data.startswith("open_"):
        idx = int(call.data.split("_")[1])
        if idx in game['opened']:
            return
        if idx in game['bombs']:
            game['opened'].extend(game['bombs'])
            send_mines_board(call.message.chat.id, user_id, bomb_triggered=True)
            del user_games[user_id]
            bot.edit_message_text("💥 Bomba topildi! Yutqazdingiz.", call.message.chat.id, call.message.message_id)
        else:
            game['opened'].append(idx)
            game['multiplier'] *= 1.08
            send_mines_board(call.message.chat.id, user_id)
...
def withdraw(message):
    ...
    if user_balances.get(user_id, 0) < 5000:
        bot.send_message(message.chat.id, "❌ Kamida 5000 so‘m bo‘lishi kerak.")
        return
...
def process_withdraw(message):
    ...
    Ha, bu holat dovdirlik (xato) bo‘libdi. Siz botga “💳 Karta raqamingizni yuboring:” so‘roviga javoban “💣 Play Mines” ni yuborgansiz, lekin bot bu xabarni haqiqiy karta raqami deb qabul qilgan.

Bu shuni bildiradi: karta raqami kiritilayotgan step handler hech qanday tekshiruv qilmayapti — ya’ni foydalanuvchi nima yuborsa, uni to‘g‘ridan-to‘g‘ri karta deb qabul qilyapti.

✅ Yechim: karta raqamini tekshirish — u raqam ekanini, uzunligi to‘g‘ri ekanini va “💣 Play Mines” kabi tugmalar emasligini aniqlash.

Quyida to‘g‘rilangan kodni beraman:

def process_withdraw(message): user_id = message.from_user.id card = message.text.strip()

# Raqam tekshiruvi: 16-19 raqamli karta bo‘lishi shart (UZCARD, HUMO, VISA)
if not card.isdigit() or len(card) < 12 or len(card) > 19:
    bot.send_message(message.chat.id, "❌ Iltimos, to‘g‘ri karta raqamini yuboring (faqat raqam).")
    return
...
bot.send_message(message.chat.id, f"✅ So‘rov qabul qilindi: {amount} so‘m karta {card} raqamiga yuboriladi.")
bot.send_message(
    ADMIN_ID,
    f"📤 Pul chiqarish:\n🆔 ID: {user_id}\n💳 Karta: {card}\n💰 Miqdor: {amount} so‘m"
)

Bu yerda isdigit() bilan raqamligini, uzunligini (kamida 12, ko‘pi 19) tekshiryapti. Endi foydalanuvchi tugma bosib yuborsa yoki matn yuborsa, xato qaytadi.

✅ Bonus: Agar siz faqat 8600... va 9860... raqamli Uzcard yoki Humo qabul qilmoqchi bo‘lsangiz, shuni ham tekshirishga qo‘shib beraman.

Aytingchi, siz qaysi turdagi kartalarni qabul qilmoqchisiz? (8600, 9860, Visa?) — shunga qarab filtr qo‘shib beraman.
